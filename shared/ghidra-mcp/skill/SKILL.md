...
Copyright the ghidra-mcp authors. License: https://polyformproject.org/licenses/noncommercial/1.0.0
The canonical source-of-truth for the driver skill lives at `shared/ghidra-mcp/skill/SKILL.md`.
Each agent plugin's binary embeds it at compile time (include_str!); regenerate that plugin's copy from the
shipped binary with `<plugin-binary> emit-skill > SKILL.md` so it can never drift. Edit ONLY that canonical
...
description: Use when reverse-engineering, triaging, or making sense of an unfamiliar binary through the ghidra-mcp Ghidra tools - reading decompiled C, navigating a call graph, or judging whether a decompile can be trusted.
---

# Driving ghidra-mcp - read, triage & edit

## Overview

ghidra-mcp attaches a persistent headless Ghidra to your session and exposes read/navigation tools plus
...
## What this tool set does NOT have
...
`inspect_function`'s `locals[]`. There are **no** composite/graph tools - `get_control_flow`,
`list_namespaces`, `trace_reachability`, `summarize_neighborhood` and `semantic_search` do not exist.
None of them are in your tool list; do not call them - they will dead-end. Where you need what they
would have given you, compose it from the primitives below.

Everything else in "The tools" above - symbol search and listing, xrefs, strings, disassembly, and
byte/type/segment/data-item reads - **is available now**. Use them directly rather than assuming
they're missing or improvising a workaround via `inspect_function` alone.
