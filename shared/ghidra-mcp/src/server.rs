...
//!
//! PINNED rmcp API (version 3.1.4) — reused by tools.rs / server.rs:
...
//!     // isError:true
//!   - text content: rmcp::model::ContentBlock::text(String) — the flat content union was
//!     renamed from `Content` (0.8) to `ContentBlock` (3.1); `CallToolResult::success`/`error`
//!     take `Vec<ContentBlock>` accordingly (`rmcp-3.1.4/src/model/content.rs`,
//!     `rmcp-3.1.4/src/model.rs` around the `CallToolResult` definition)
...
//!
//! Verified by compiling this crate against the real, resolved `rmcp = "3.1.4"` (workspace pin
//! `rmcp = "3.1"`) and by reading the registry source under
//! `rmcp-3.1.4/src/{lib.rs, model.rs, model/content.rs, handler/server/tool.rs,
//! handler/server/router/tool.rs, handler/server/wrapper/parameters.rs}` — see
//! `shared/ghidra-mcp/Cargo.toml`.
//!
//! Additional API facts nailed down while porting Tasks 15/16 from 0.8.5 to 3.1.4 (kept here so
//! the pinned block stays the single source of truth):
//!   - `#[tool]`-annotated methods must return a type implementing `IntoCallToolResult`. The
//!     blanket impls cover `T: IntoContents` (Content/String/()), `CallToolResult` itself,
//!     `rmcp::ErrorData` itself, and `Result<T, E>` where both `T` and `E` are
//!     `IntoCallToolResult` (verified in `rmcp-3.1.4/src/handler/server/tool.rs`) — the same
//!     shape as 0.8. A bare `-> CallToolResult` return does NOT satisfy the trait. Every tool
//!     method here therefore returns `Result<CallToolResult, rmcp::ErrorData>` and always yields
//!     `Ok(..)` — domain failures are encoded as `CallToolResult::error(..)` inside the Ok, never
//!     as the method's own `Err`.
//!   - `#[tool_router]`/`#[tool]` attribute values are parsed by `darling`'s `FromMeta`, whose
//!     default `from_expr` only accepts `Expr::Lit` (a literal) or `Expr::Group`; a bare
//!     identifier/path (e.g. a `const DESC: &str` referenced as `description = DESC`) is
//!     `Expr::Path` and is rejected at macro expansion. So `#[tool(description = "...")]` needs
//!     the literal text inline; the `DESC_*` consts in `tools.rs` stay the single authored copy
//!     of the wording and this file's literals are byte-for-byte copies of them (checked by the
//!     `tool_descriptions_match_tools_rs_consts` regression test below).
//!   - `schemars` is a DIRECT dependency of this crate (`Cargo.toml`'s
//!     `schemars = { workspace = true }`), so the `#[derive(schemars::JsonSchema)]` on the arg
//!     structs below resolves the bare `schemars::` path through Rust's 2021-edition extern-crate
//!     prelude — no `use rmcp::schemars;` needed. (rmcp 3.1 still re-exports `schemars` at
...
//!     `router::tool`, not the bare `tool` module — confirmed via
//!     `grep -rl "struct ToolRouter" rmcp-3.1.4/src/handler`). `#[tool_handler]`'s generated
//!     `call_tool`/`list_tools` default to calling `Self::tool_router()` fresh rather than
//!     reading `self.tool_router` (`rmcp-macros-3.1.4/src/tool_handler.rs`), so the stored field
//!     itself goes unread — `#[allow(dead_code)]`'d below rather than removed, to keep the
//!     conventional shape this workspace's other MCP servers use.
//!   - the client-side call-tool params struct (used only by tests/protocol.rs) was renamed from
//!     `rmcp::model::CallToolRequestParam` to `CallToolRequestParams` and marked
//!     `#[non_exhaustive]` with new MRTR/SEP-2322 fields (`input_responses`, `request_state`);
...
pub struct GhidraMcpServer {
    ...
    // Populated by `new()` via `Self::tool_router()`, but the `#[tool_handler]`-generated
    // `call_tool`/`list_tools` methods call `Self::tool_router()` fresh each time rather than
    // reading this field (confirmed in `rmcp-macros-3.1.4/src/tool_handler.rs`: the default
    // `router` expression is the `Self::tool_router()` call, not `self.tool_router`), so nothing
    // in this crate currently reads it back. Kept as the conventional `#[tool_router]` field
    // (rmcp's own examples and this workspace's other MCP servers all carry one) in case a
    // future caller needs the constructed instance directly.
    #[allow(dead_code)]
...
// `router = self.tool_router` is NOT optional here, and its absence is silent.
//
// In rmcp 0.8 the attribute defaulted to the `self.tool_router` FIELD. In 3.1 it
// defaults to the expression `Self::tool_router()`, which CONSTRUCTS A FRESH
// ROUTER on every `call_tool`, `list_tools` and `get_tool` — rebuilding all 19
// tool descriptors and their JSON schemas per request. It still works, which is
// the problem: the only outward sign is that the field built once in `new()`
...
mod tests {
    ...
    fn tool_descriptions_match_tools_rs_consts() {
        let list = GhidraMcpServer::list_project_programs_tool_attr();
        let attach = GhidraMcpServer::attach_program_tool_attr();
        let inspect = GhidraMcpServer::inspect_function_tool_attr();
        let find = GhidraMcpServer::find_functions_tool_attr();
        let resolve = GhidraMcpServer::resolve_symbol_tool_attr();
        let list_symbols = GhidraMcpServer::list_symbols_tool_attr();
        let list_strings = GhidraMcpServer::list_strings_tool_attr();
        let get_xrefs = GhidraMcpServer::get_xrefs_tool_attr();
        let get_disassembly = GhidraMcpServer::get_disassembly_tool_attr();
        let read_bytes = GhidraMcpServer::read_bytes_tool_attr();
        let get_datatype = GhidraMcpServer::get_datatype_tool_attr();
        let list_segments = GhidraMcpServer::list_segments_tool_attr();
        let list_data_items = GhidraMcpServer::list_data_items_tool_attr();
        let describe_address = GhidraMcpServer::describe_address_tool_attr();
        let rename = GhidraMcpServer::rename_tool_attr();
        let comment = GhidraMcpServer::comment_tool_attr();
        let set_datatype = GhidraMcpServer::set_datatype_tool_attr();
        let set_prototype = GhidraMcpServer::set_prototype_tool_attr();
        let set_local = GhidraMcpServer::set_local_tool_attr();
        assert_eq!(
