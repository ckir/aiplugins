//! Embed the driver skill (`skill/SKILL.md`) in the binary so an agent plugin can regenerate its
//! committed copy from the shipped binary (single source of truth — no skill drift). The ship-guard
//! test below fails the build if the embedded content is stubbed, emptied, or truncated rather than the
//! authored driver skill.
//!
//! The skill lives INSIDE this crate rather than at a repo root, so `include_str!` needs no `..`
//! traversal and the crate stays relocatable: `shared/ghidra-mcp` can be vendored or moved without
//! silently embedding whatever happens to sit at the old relative path.

/// The driver skill, embedded at compile time (version-locked to this binary).
pub const DRIVER_SKILL: &str = include_str!("../skill/SKILL.md");

/// The bytes a plugin's `emit-skill` subcommand writes: the canonical with its leading HTML comment header removed,
...
mod tests {
    ...
    /// pinned separately by each plugin's tests/skill_emit.rs, which compares the built binary's stdout to
    /// that plugin's committed copy. Reads the
    ...
    fn embedded_skill_matches_disk_source() {
        let disk = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/skill/SKILL.md"))
