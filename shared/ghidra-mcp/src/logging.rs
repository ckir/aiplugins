    ...
    impl Drop for LocalFreeGuard {
        fn drop(&mut self) {
            if !self.0.is_null() {
                ...
                // freed elsewhere.
                //
                // `windows` 0.62 takes `Option<HLOCAL>` here where 0.58 took a bare `HLOCAL`;
                // `None` is the API's documented no-op, and the null check above already
                // covers that case, so this is always `Some`.
                unsafe {
...
mod tests {
    ...
    fn init_creates_owner_only_log_dir_and_never_panics() {
        let base = std::env::temp_dir().join(format!("ghidra-mcp-log-{}", std::process::id()));
