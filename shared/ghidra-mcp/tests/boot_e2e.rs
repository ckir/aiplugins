...
//! a known function, get C back. Skipped unless GHIDRA_MCP_E2E=1 and the fixture env vars are set.
...
async fn boot_attach_decompile_end_to_end() {
    if env("GHIDRA_MCP_E2E").as_deref() != Some("1") {
        eprintln!("skipping boot_e2e (set GHIDRA_MCP_E2E=1 + fixture env vars)");
        return;
    }
    let script_dir = std::env::temp_dir().join("ghidra-mcp-e2e-scripts");
    ghidra_mcp::worker_asset_extract(&script_dir);

    // `GHIDRA_MCP_FIXTURE_PROGRAM` is the program's absolute VFS path (e.g. `/add.exe`) — the form
    ...
    let program = env("GHIDRA_MCP_FIXTURE_PROGRAM").expect("PROGRAM");
    ...
    let cfg = WorkerConfig {
        ...
        project_dir: PathBuf::from(env("GHIDRA_MCP_FIXTURE_PROJECT_DIR").expect("PROJECT_DIR")),
        project_name: env("GHIDRA_MCP_FIXTURE_PROJECT_NAME").expect("PROJECT_NAME"),
        bootstrap_program: process_name,
        script_dir,
        script_name: ghidra_mcp::worker_asset::WORKER_SCRIPT_NAME.to_string(),
        max_heap: None,
    ...
    let func = env("GHIDRA_MCP_FIXTURE_FUNCTION").unwrap();
    let dec = w
        .conn
        .request(&req(
            2,
