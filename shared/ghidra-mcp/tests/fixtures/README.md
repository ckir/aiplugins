...
3. Point the tests at it with env vars (run the suite from **PowerShell**, `-j1`):
       ...
       GHIDRA_MCP_FIXTURE_PROJECT_DIR=<fixtureDir e.g. C:\Users\user\AppData\Local\Temp\ghidra-mcp-fix>
       GHIDRA_MCP_FIXTURE_PROJECT_NAME=fixtureproj
       GHIDRA_MCP_FIXTURE_PROGRAM=/fixture.exe
       GHIDRA_MCP_FIXTURE_FUNCTION=compute

Without `GHIDRA_MCP_E2E=1` the tests return early (skipped).
