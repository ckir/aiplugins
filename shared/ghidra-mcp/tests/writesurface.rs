// M2b write-surface e2e. Gated by GHIDRA_MCP_E2E=1. Run from PowerShell, -j1 (single JVM per worker):
//   $env:GHIDRA_MCP_E2E="1"; cargo nextest run -p ghidra-mcp --test writesurface -j1
mod common;
use common::*;
...
async fn data_refs_excludes_stack_slots() {
    ...
    let (_dir, state) = fixture_state_ephemeral();
    // inspect_function's param key is `function` (verified in GhidraMcpWorker.resolveFunction), not
