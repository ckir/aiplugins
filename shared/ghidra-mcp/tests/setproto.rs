// M2c set_prototype e2e. Gated by GHIDRA_MCP_E2E=1. Run from PowerShell, -j1 (single JVM per worker):
//   $env:GHIDRA_MCP_E2E="1"; cargo nextest run -p ghidra-mcp --test setproto -j1
mod common;
use common::*;
use ghidra_mcp::state::ServerState;
use std::sync::Arc;

// Fixture facts discovered LIVE (Task 7 Step 0) against the ghidra-mcp-fix build, real Ghidra 12.1.2, via a
...
async fn set_prototype_boundaries() {
    ...
    // (d) a THUNK/EXTERNAL import and (e) a custom-storage function: the ghidra-mcp-fix build statically
