...
//! return the RIGHT structured ErrorCode. Gated on GHIDRA_MCP_E2E=1 + fixture env (real Ghidra).

mod common;
use common::{call_when_warm, enabled, fixture_state};
use ghidra_ipc::error::ErrorCode;
use ghidra_mcp::execute::{attach_program, call_worker};

#[tokio::test]
async fn inspect_function_returns_real_c_on_bootstrap_program() {
    if !enabled() {
        eprintln!("skip: set GHIDRA_MCP_E2E=1 + fixture env");
        return;
    }
    let state = fixture_state();
    state.start_warmup().await;
    let func = common::env("GHIDRA_MCP_FIXTURE_FUNCTION").expect("FIXTURE_FUNCTION");
    ...
    let fc = ghidra_mcp::tools::canonicalize_function_context(&canon, fc);
    assert!(
...
async fn crucible_domain_errors() {
    ...
    // NOTE: suggested_action canonicalization (server.rs GhidraMcpServer::err_canon) is applied by the
