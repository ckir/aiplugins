//! The oracle nobody had: `re-ghidra-cc-mcp emit-skill` must reproduce the COMMITTED plugin copy byte for byte.
//!
...
//!
//! This is the FAST tier (`just test`, pure Rust, no Ghidra and no JVM): `emit-skill` only prints an
//! embedded `include_str!` const. Putting it behind the live-Ghidra gate would tie the emit contract to
//! GHIDRA_INSTALL_DIR + JDK 21, so it would almost never run.
//!
//! It lives in the PLUGIN rather than in `shared/ghidra-mcp` because the artifact under test is this
//! plugin's committed copy. The canonical skill is shared; each agent plugin owns — and must pin — its
//! own generated copy of it.
...
/// time - no `cargo run` indirection, and no dependence on anything being installed on PATH.
fn emit() -> Vec<u8> {
    let out = Command::new(env!("CARGO_BIN_EXE_re-ghidra-cc-mcp"))
        .arg("emit-skill")
        .output()
        .expect("run `re-ghidra-cc-mcp emit-skill`");
    assert!(
        out.status.success(),
        "`emit-skill` exited {:?}: {}",
        out.status.code(),
