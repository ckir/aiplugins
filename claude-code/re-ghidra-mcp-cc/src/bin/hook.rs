//! `re-ghidra-cc-hook` — the plugin's `SessionStart` preflight.
//!
...
use re_ghidra_mcp_cc::{load_settings, parse_java_major, preflight, Probe};
use std::path::{Path, PathBuf};
use std::process::Command;

fn main() {
    // Draining stdin is not optional. Claude Code writes the event JSON to the
    ...
    let body = format!(
        "re-ghidra-mcp-cc is not ready to attach to a Ghidra project:\n{}",
        findings
            .iter()
            .map(|f| format!("  - {f}"))
            .collect::<Vec<_>>()
    ...
    // lands whichever one the running Claude Code honours.
