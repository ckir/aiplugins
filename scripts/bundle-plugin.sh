...
# about platforms:
#
#   * On Windows the spawn resolves `bin/<name>` through PATHEXT and lands on
#     `bin/<name>.exe` — the extensionless dispatcher sitting next to it is
#     never chosen, because libuv tries .com and .exe before the bare name.
...
# One dispatcher per binary. `exec` keeps the pid and the stdio handles, which
# an MCP server speaking JSON-RPC over stdin and stdout depends on.
for name in $binaries; do
    sed "s/__NAME__/$name/g" > "$stage/bin/$name" <<'DISPATCH'
#!/bin/sh
# Generated by scripts/bundle-plugin.sh — do not edit.
#
# Runs the binary built for this machine. Windows never reaches this file: the
# spawn resolves the sibling .exe first.
set -e
