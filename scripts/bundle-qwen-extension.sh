#!/usr/bin/env bash
#
# Assemble one self-contained, installable zip for a Qwen Code extension.
#
# `qwen extensions install` fetches an archive URL and extracts it as the
# extension directory. Nothing in that flow compiles anything, so the zip has
# to carry the binaries `.qwen-extension.json` points at — for every platform,
# because a marketplace entry is ONE url with no per-platform matrix.
#
...
#
# The manifest keeps naming `${extensionPath}/bin/<name>` and never learns
# about platforms:
#
...
#     the running uname. Qwen's zip installer reads unix modes out of the zip
#     central directory and chmods anything carrying an exec bit, so the
...
# attributes instead, the exec bits are lost, and the bundle installs onto
...
#
# Usage: scripts/bundle-qwen-extension.sh <extension-name> <assets-dir> <out-dir>
#
#   <assets-dir>  holds the per-target release archives dist already publishes:
#                 <extension>-<target>.tar.xz, and <extension>-<target>.zip on Windows.
#   <out-dir>     receives <extension>-extension.zip, built from a staging tree of the
#                 same name that is left in place for inspection.
set -euo pipefail

if [ "$#" -ne 3 ]; then
    echo "usage: $0 <extension-name> <assets-dir> <out-dir>" >&2
    exit 2
fi
...
case $(uname -s) in
    MINGW* | MSYS* | CYGWIN*)
        echo "ERROR: qwen extension bundles cannot be built from a Windows shell." >&2
        echo "       Use WSL, Linux, or macOS — see the comments in this script." >&2
        exit 1
        ;;
esac

[ -f "$src/qwen-extension.json" ] || {
    echo "ERROR: $src/qwen-extension.json not found — is '$ext' a Qwen extension?" >&2
    exit 1
}
...
# The binaries to ship are the ones the extension's manifest names. Deriving them
# from the manifest, rather than from whatever the archives happen to contain,
# makes this the same contract every installer enforces: every
# `${extensionPath}/bin/X` reference has to end up as a real file in bin/.
binaries=$(
    jq -r '
...
[ -n "$binaries" ] || {
    echo "ERROR: $ext declares no \${extensionPath}/bin/... commands to bundle." >&2
    exit 1
}
...
# Copy the extension tree, minus what only the build needs. Excluding, rather than
# listing what to include, means a component directory added later — skills/,
# examples/ — ships without anyone remembering to edit this script.
tar -c -C "$src" \
    --exclude=./bin \
    --exclude=./src \
    --exclude=./tests \
...
# The extension directories carry no LICENSE of their own; the repo's applies.
cp "$repo/LICENSE" "$stage/LICENSE"

for target in $targets; do
    case $target in
        *windows*) archive="$assets/$ext-$target.zip" ;;
        *) archive="$assets/$ext-$target.tar.xz" ;;
    esac
    ...
    case $target in
        *windows*)
            ...
            ;;
        *)
            # dist's unix tarballs wrap everything in one <ext>-<target>/ dir.
            tar -xf "$archive" -C "$extract"
            root="$extract/$ext-$target"
            [ -d "$root" ] || {
                echo "ERROR: $(basename "$archive") has no $ext-$target/ directory" >&2
                exit 1
            }
...
printf '%s\n' "$listing" | grep -qxF 'qwen-extension.json' ||
    report "qwen-extension.json"
